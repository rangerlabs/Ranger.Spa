...
const swiftApiClientV1Descriptions = [
    {
        title:
            'RangerSwiftApiClientV1.PostBreadcrumb(breadcrumb: Breadcrumb, apiKey: String, completionHandler: @escaping ((RangerApiResponse<Data>) -> Void))',
...
const swiftTrackerV1Descriptions = [
    ...
    },
    {
        ...
        description:
            'Calls requestAlwaysAuthorization() on the underlying CLLocationManager, must be called before tracking if requesting Always authorization',
    },
    {
            ...
            <Section text="Swift" id="swift-section">
                ...
                <SubSection text="Swift Tracker v1" id="swift-tracker-v1-section">
                    ...
                    <SubSection text="Installation and usage" id="swift-tracker-v1-install-section">
                        ...
                        <Paragraph>
                            Before calling any tracking methods on the Tracker, it is necessary to call <Code>RangerTrackerV1.configure()</Code> to configure
                            the Tracker. Once configured, your application may request authorization using either{' '}
                            <Code>RangerTrackerV1.requestWhenInUseAuthorization()</Code> or <Code>RangerTrackerV1.requestAlwaysAuthorization()</Code>. You can
                            then begin tracking of the device by calling either <Code>RangerTrackerV1.trackStandardLocation()</Code> or
                            <Code>RangerTrackerV1.trackSignificantLocationChanges()</Code>.
                        </Paragraph>
                        <Paragraph>
                            The Tracker also includes the <Code>PusherNotifier</Code>. The PusherNotifier can be configured to connect to the Pusher Channel
                            defined in your Pusher Integration. The <Code>PusherNotifier.connect()</Code> method allows for a callback handler to defined and
                            will execute whenever a Geofence Event occurs. Within the callback handler it's possible to raise notifications, alerts, or begin
                            location specific in-app actions. For an example on how to use the <Code>PusherNotifier</Code>, visit the Swift Demo repository on
                            our{' '}
                            <Link
                                component="button"
                        ...
                        <Paragraph>
                            Remember, when requesting location services on iOS, it is necessary to a meaningful Purpose String in the <Bold>Info.plist</Bold>{' '}
                            file. When requesting When In Use authorization or Always authorization, set a meaningful value for
                            NSLocationWhenInUseUsageDescription. When requesting Always Authorization set a meaningful value for
