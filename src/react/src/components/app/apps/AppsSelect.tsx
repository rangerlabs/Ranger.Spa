...
class AppsSelect extends React.Component<AppsSelectProps> {
    ...
    handleAppClick(app: IApp) {
        this.props.apps.filter(a => a.id === app.id);
        this.props.selectApp(app);
        const redirect = this.getRedirectFromParams();
        if (redirect) {
            this.props.push('/' + app.name + redirect);
        } else {
            this.props.push(RoutePaths.GeoFenceMap.replace(':appName', app.name));
        }
    }
                        ...
                        <Parallax pages={2} scrolling={true}>
                            <ParallaxLayer speed={0.7}>
                                <Grid container spacing={3} direction="row" justify="center" alignItems="baseline">
                                    {apps.map(app => (
                                        <Grid key={app.id} item xs={12} sm={8} md={7}>
                                            <ButtonBase
